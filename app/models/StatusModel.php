...
class StatusModel {
    ...
    public function list($start = 0, $length = 10, $search = '', $colIndex = 2, $orderDir = 'desc') {
        $sqlTotal = "SELECT COUNT(*) FROM wp_project_status WHERE status <> 'deleted'";
        if (!empty($search)) { $sqlTotal .= " AND project_status_name LIKE :search"; }
        $stmtTotal = $this->db->prepare($sqlTotal);
        if (!empty($search)) { $stmtTotal->bindValue(':search', '%' . $search . '%'); }
        $stmtTotal->execute();
        $total = (int)$stmtTotal->fetchColumn();
        $orderMap = [1 => "project_status_name", 2 => "created_at"];
        $order = $orderMap[$colIndex] ?? 'created_at';
        $orderDir = strtolower($orderDir) === 'asc' ? 'asc' : 'desc';
        $sql = "SELECT project_status_id, project_status_name, project_status_color, created_at FROM wp_project_status WHERE status <> 'deleted'";
        if (!empty($search)) { $sql .= " AND project_status_name LIKE :search"; }
        ...
        foreach ($rows as &$row) { $this->formatDocumentRow($row); }
    ...
    public function save($data) {
        ...
        if ($id) {
            ...
            $sql = "INSERT INTO wp_project_status (project_status_name, project_status_color, status, created_at, updated_at) 
                    VALUES (:name, :color, 'active', NOW(), NOW())";
            $stmt = $this->db->prepare($sql);
        }
